...
if Ops.flagstim
    ...
    son = find(Data.stim_on);
    soff = find(abs(Data.stim_on-1)); 
    onInd = SplitVec(son,'consecutive');
    offInd = SplitVec(soff,'consecutive');
    CycleInStimOn = cellfun(@(x) ArrContainsArr(son,x),GCycles);
    CycleInStimOff = cellfun(@(x) ArrContainsArr(soff,x),GCycles);
    GoodCycles = GCycles(logical(CycleInStimOff));
    StimCycles = GCycles(logical(CycleInStimOn));
else
    GoodCycles = GCycles;
end
CycleLength = cellfun(@(x) length(x),GoodCycles);
%%
PCycleInStimOn = cellfun(@(x) length(intersect(x,son))/length(x),GoodCycles);
PCycleInStimOff = cellfun(@(x) length(intersect(x,soff))/length(x),GoodCycles);
CIS = [PCycleInStimOff' PCycleInStimOn'];

%% Compute Neuron Phase During each Cycles
locMax = [];
for j = 1:size(Data.NormUFiring,2)
    [~,loc] = cellfun(@(x) findpeaks(Data.NormUFiring(x,j),'NPeaks',1,'MinPeakHeight',std(Data.NormUFiring(x,j))),GoodCycles,'UniformOutput',false);
...
for S = 1:length(SpeedInd)
    MeanSpeeds = cellfun(@(x) GetResampSig(PC(x,:),2000),GoodCycles(SpeedInd{S}),'UniformOutput',false);
...
%% Compute Tangling 
Tanglingoff = cellfun(@(x) sum(ComputeTrajectoryTangling(PC(x,1:3),1,1/fs)), GoodCycles); 
Tanglingon = cellfun(@(x) sum(ComputeTrajectoryTangling(PC(x,1:3),1,1/fs)), BadCycles); 
...
hold on
col = {Colors().BergGray09, Colors().BergBlue};
width = 0.5;
 for jj = 1:size(CIS,2)
    Std = std(CIS(:,jj));
    mn = mean(CIS(:,jj));
    patch([jj-width/2 jj+width/2 jj+width/2 jj-width/2],[mn-Std mn-Std mn+Std mn+Std],col{jj}*(1/jj),'FaceAlpha',0.7);  
    scatter(ones(length(CIS(:,jj)),1)*jj,CIS(:,jj),100,'MarkerEdgeColor',col{jj},'MarkerFaceColor',col{jj}*(1/jj),'MarkerFaceAlpha',0.5,'LineWidth',0.5)
end
xlim([0 size(CIS,2)+1]);
xticks([1:size(CIS,2)]);
...
subplot(2,2,3)
hold on
cellfun(@(x,y) histogram(Ops.fs./CycleLength(x),'BinWidth',quantile(Ops.fs./CycleLength,0.1)-quantile(Ops.fs./CycleLength,0.05),'EdgeColor','white','FaceColor',[Colors().BergBlue/y],'FaceAlpha',0.75),SpeedInd,lin)
...
subplot(2,2,4)
hold on
cellfun(@(x,y) scatter(Ops.fs./length(GoodCycles{x}),mean(vecnorm((PC(GoodCycles{x},1:3)-mean(PC(GoodCycles{x},1:3),1))')),50,'MarkerFaceColor',Colors().BergBlue/y,'MarkerEdgeColor','none','MarkerFaceAlpha',0.65),num2cell(cell2mat(SpeedInd)),num2cell(col))

%cellfun(@(x,y) scatter(Ops.fs./length(GoodCycles{x}),sum(mad(PC(GoodCycles{x},1:3),0,1)),50,'MarkerFaceColor',Colors().BergBlue/y,'MarkerEdgeColor','none','MarkerFaceAlpha',0.65),num2cell(cell2mat(SpeedInd)),num2cell(col))
xlabel('Cycle Frequency [Hz]');
ylabel('Mean Absolute Difference');
title('Cycle Duration VS Mean Absolute Deviation');
%save2pdf(fig,[FileContent(1).folder filesep 'Figures'],['SpeedDistributionAndTrajectories']);
...
fig = figure('Color','white','Position',[0,0,Ops.screensize(3)/4, Ops.screensize(4)]);
hold on 
der = cellfun(@(x) max(diff(PC(x(1:100:end),1))),GoodCycles);
der = der./max(der);
lin_ind = 1:length(der);
cellfun(@(x) plot3(PC(x(1:100:end),1),PC(x(1:100:end),2),PC(x(1:100:end),3),'LineStyle','-','Color',[Colors().BergGray09 0.3],'LineWidth',1),GoodCycles);
cellfun(@(x) scatter3(PC(x(1),1),PC(x(1),2),PC(x(1),3),20,'MarkerFaceColor',Colors().BergBlue,'MarkerEdgeColor','none'),GoodCycles);
cellfun(@(x) scatter3(PC(x(end),1),PC(x(end),2),PC(x(end),3),20,'MarkerFaceColor',Colors().BergYellow,'MarkerEdgeColor','none'),GoodCycles);
...
%% Plot Cycles in PC spaces
fig = figure('Color','none','Position',[0,0,Ops.screensize(3)/2, Ops.screensize(4)]);
ax1 = axes('Color', 'none');
hold on 
der = cellfun(@(x) max(diff(PC(x(1:100:end),1))),GoodCycles);
der = der./max(der);
...
for Speed = SpeedInd
    ...
    cellfun(@(x) surfx(PC,x), GoodCycles(Speed{1}));
%     index = index +1;
end
...
%set(gcf,'Children',[ax2 ax1])
%linkaxes([ax1,ax2]);
% axis equal
% axis square
%save2pdf(fig,[Folder filesep 'Figures'],['CyclesInPCSpace_StimNoStimSpeed'],'-dpng')
%%
fig = figure('Color','white','Position',[0,0,Ops.screensize(3)/4, Ops.screensize(4)]);
hold on 
...
fig = figure('Color','white','Position',[0,0,Ops.screensize(3)/1, Ops.screensize(4)/5]);
% Smooth Firing Rate
% ax1 = subplot(2,1,1);
...
Raster = Data.USpiking(:,SortingIndices)';
